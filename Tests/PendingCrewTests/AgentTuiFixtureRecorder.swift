...
///
/// ## 全量里的 skip 基线：记构成，别记数字
///
/// 跑全量看到「N tests skipped」时，**别拿一个记住的数字去对**。当前构成是：
///
/// - `CrewLastMessageCacheTests.test_基准_现场白板目录`
/// - `SessionAwaitingReplyInputsCacheTests.test_基准_现场目录`
/// - **本录制器**（`testRecord`，见上：工具不是测试，**永久 skip**）
///
/// **数字会随刻意增删漂，构成不会。** 读到不符时，先去看这几条各自还在不在 ——
/// 别默认是回归。
///
/// > **`8561c53` 的提交信息里写着「skip 基线 3 → 4」，那句是错的，别照着用。**
/// > 它是在「旧基线 3 是常数」这个前提下 +1 算出来的，而同一批改动里
/// > #63 第二期把登录那一层整个删了，**原来那 3 条里的 Keychain 那条随之消失**。
/// > 3 − 1 + 1 = 3。提交信息改不了，所以更正留在这儿。
/// > （教训不是「算错了」，是**拿一个不是这一趟亲眼看到的数当判据** ——
/// > 同一天里这个形状咬了两次：另一次是一组「全绿但其实什么都没测到」的边角用例。）
@MainActor
