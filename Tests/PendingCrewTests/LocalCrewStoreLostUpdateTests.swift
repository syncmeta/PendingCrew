...
///
/// ## ⚠️ 这一组**证不到锁那一半**，别把绿当成两半都对
///
/// 修法有两半：**锁内**、**重读→改→写**。这一组只钉得住后一半。实测（负向对照）：
///
/// | 把哪一半拿掉 | 这一组的反应 |
/// |---|---|
/// | 拿掉**重读**（锁还在） | **4 条红** —— 病根就在这半，真机上撞到的也是它 |
/// | 拿掉**锁**（重读还在） | **全绿** —— 红不到 |
///
/// 为什么红不到：这里的两个 store 实例在**同一个进程、同一条线程**上顺序执行，
/// 压根没有真并发。而且想在这一层造并发也造不出红 —— 同线程对**不同 fd** 再取一次
/// `flock` 会**死锁**，那是挂住，不是红。
///
/// 锁那一半的背书在 `MultiProcessJSONStore.withFileLock` 自己那儿，不该由这条测试
/// 重新证一遍；**刻意不补一条会 flaky 的并发测试** —— 一条时好时坏的尺子和一条
/// 永远报「没事」的尺子是同一种毒：它会训练人忽略它。
///
/// 另一半的边界：两个实例**不等于**两个真进程（真进程还有调度、页缓存、崩溃时机）。
/// 真进程那一半是手工复现的，进不了回归网 —— 机长在真 `--daemon` 上先复现了丢更新，
/// 修完又用同一套靶子复跑，外部那次改动这回活了下来。
/// 两者互补：这一组证明它**必然**发生，那两次证明它**真的**发生过、也**真的**修好了。
@MainActor
