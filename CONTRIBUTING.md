...
| **量错对象**：指针的形式一直有效，指的东西早就走了 | `docs/tech-debt.md` 引 `CONTRIBUTING.md:271`「真实群聊数据不入版本历史」，**写下时逐字正确**；两次追加把那句话顶到 367、本节改索引又挪到 258，而 271 行**一直存在**（后来才越过文件末尾）—— 跳过去的人只会读到一句不相干的话，然后接着往下走 | 提交 `1f8cc7f` |
