...
**同一种沉默的第二次，这次真出事了（2026-08-26，`140cb0f`）**：Todo #12 那笔的基线在
0.1.16 发版之前，pbxproj 里是 `MARKETING_VERSION = 0.1.15`；发版提交把它改成 `0.1.16`。
**git 自动合掉两边、零冲突、零报错、编译照过 —— 合出来的是 `0.1.15`。** 抓到它的不是
git，是合完那一次 `scripts/gen-project.sh` 重生对字节（第 1 条）。**上面那次是「核了发现
没事」，这次是「核了发现是发版号」** —— near-miss 单独摆着像是「其实也没事」，配上这次
才看得出它是差一点。

#### 实例五：尺子量对了性质，量错了对象

前四个是「看不见目标」或「看的是另一件事」。**这一个看的是对的那件事，只是那件事属于
另一个对象。**

- **发版链**（2026-08-26）：`build-macos-update.sh` 取的是 `main` 而不是某个 ref，**而且取
  两次**（建快照一次、打 tag 一次），中间隔着十几分钟公证。**「闸门跑的 / 真被构建的 /
  tag 指向的」可以是三棵不同的树，而没有任何一步会报警。** 那次它们相等，靠的是发版期间
  main 被冻结 —— **那道冻结不是流程礼节，它就是让三者相等的机制。**
- **共享目录**：在共享目录 `git checkout -b` 之后，闸门四条读数（HEAD 前后一致 / 脏指纹
  一致 / 零具名失败 / 那 8 条真跑了）**一条都不会响**，`git status` 也干净 —— 脏文件显形，
  切分支不显形。四条读数量的是「这棵树跑得对不对」，**不是「这棵树是不是那棵树」**。
...
**配套的两条操作规矩**：**共享目录恒在 `main`，要开分支就开 worktree**；跑判据用的全量
**开一个钉死在目标 commit 的 detached worktree**（`git worktree add --detach`），让 HEAD
在物理上没法在脚下移动 —— 事后检查靠的是「在正确的时刻想起来做一件正确的事」，而那
恰恰是最不可靠的一环。

#### 实例六：消音器是你自己装的

前五个都是尺子有问题。**这一个尺子没问题** —— 是使用者在自己和原始输出之间加了一层省略，
然后拿省略后的结果当全貌。三个现场都在 2026-08-26 同一天：

- **汇总行**：`Executed N tests, with M failures` 说红了几条，**恰恰不说是哪一条**（第 2 条）
- **`head -20`**：把 skip 构成 `1+8+2` 压成看不见的后两项，于是「构成」变成了 `11-1=10`
  这个减法 —— **一个数字穿上了构成的外衣**
- **`grep -c`**：两份模板 `broadcast` 命中 `zh 0 / en 4`，差一点报成「en 有、zh 漏了」；
  回去逐条看那 4 行，**全是英文里 broadcast 这个普通词，两份都没写**

> **任何在你和原始输出之间做省略的东西 —— `head` / `tail` / `-c` / 汇总行 / 摘要 ——
> 一旦它的输出要被当成判据，就必须摘掉。** 多几行不值钱；**一个被压扁的构成会让你把
> 减法当成事实。**

**推论**：记构成别记数字，**而构成必须是逐条枚举出来的、带名字**。任何靠总数减出来的
分项都不算构成，哪怕算术是对的。
