...
#### 实例四：git 说「没冲突」，量的是文本打不打架，不是语义还在不在

前三个实例是**尺子看不见目标**。这一个不一样：**尺子看得很清楚，只是它看的是另一件事。**

Todo #63 第二期是一次**整层删除**（`CrewRelayAgent` / `EdgeBackend` / 交互卡整套 /
`postCrewMessage` 的 `attachmentIds` 形参…）。Todo #60 那条线的四处 hunk 在它后面
rebase，**自动合、零冲突**。

**零冲突只说明文本没打架，不说明那几行还落在它该落的地方。** 一段改动完全可能干干净净
地贴在一段**已经没有意义的上下文**旁边 —— 周围的类型没了、调用方没了、那个分支再也走不到，
而 git 一个字都不会说，因为从文本上看那里根本没有人跟你抢。

#60 逐处打开核过：**四处全部完好、相对位置也对。** 结果是好的 —— 但**这个结果是核出来的，
不是合出来的**。

**所以：大面积删除之后，「git 没报冲突」不是证据，「我逐处看过」才是。** 报告里只写前半句
的，等于用一把量文本的尺子回答了一个关于语义的问题。
