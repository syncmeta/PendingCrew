...
| **量错对象**：指针的形式一直有效，指的东西早就走了 | `docs/tech-debt.md` 引 `CONTRIBUTING.md:271`，而全文只有 268 行 —— **271 > 268，越过了文件末尾**。零判断就能判死，而它在那儿躺了很久没人发现：271 行"一直存在"，跳过去只会读到一句不相干的话。全仓同类 6 处，**其中 2 处（271 > 268、640 > 514）是靠 `N > wc -l` 抓的，另有 1 处从写下那天就指错了文件** | 提交 `1f8cc7f` + `7523044`（两笔合起来才是全部六处） |
