...
| # | 验收项 | 判定方式 |
|---|---|---|
| A1 | 更新 app 不打断任何在跑的 session | §9 端到端剧本 |
...
不选它的代价：要自己写握手、心跳、重连。这部分是有限且已知的（§4.4）。
...
| 白板 / Todo / 审批 / 会话快照的**写入** | daemon（MCP helper 子进程照旧直写共享文件，不变） |
| 所有唤醒器、中继、额度/模型轮询 | daemon |
| 编排（派工、建 crew、起机长、拍板转发） | daemon |
...
语料必须覆盖：SGR 全家（含 256 色 / 真彩 / 下划线样式）、CJK 宽字符、组合字符、滚动区域（DECSTBM）、alt-screen 进出、清屏、绝对/相对光标移动、超出 scrollback 上限的溢出。
...
- daemon 持续把 `{sessionId, pid, pgid, 启动时刻}` 写进 `sessions.registry.json`。
- 由系统守护自动拉起后，daemon 第一件事：读 registry → 对每个记录在案但已不属于自己的 pid，**主动 kill 掉整个进程组**（防止半死的孤儿继续写文件、继续烧额度）。
- 然后往受影响的每个 crew 白板发一条：「后台进程重启，N 个 session 被中断」，逐个点名。**fail loud，不静默。**

### 8.3 版本不一致（新 app + 旧 daemon）

握手交换 `protocolVersion`（只在不兼容变更时才 +1）与 `build`。

| 情况 | 行为 |
|---|---|
| 版本相同 | 正常连接（build 不同不管） |
...
| **P5**<br>常驻与善后 | 系统级后台服务注册（开机自启 + 崩溃自拉）；菜单栏常驻项；版本握手横幅；孤儿回收；A1 端到端剧本；把总闸默认切成 `daemon` | A1–A4 全绿；#59 的性能采样复跑，证明未 attach 的 session 主线程开销为零 | 总闸设回 `inproc`（daemon 停掉即可） |
...
1. **后台怎么常驻** —— 推荐注册成系统级后台服务（开机自启、崩溃自拉、关 app 仍活）；代价是「系统设置 → 登录项」里多一条。备选：app 拉起后脱离独活，不进登录项，但整机重启后要等下次开 app。
