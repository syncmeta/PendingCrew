...
- **恢复：12:16:19 到 12:16:29 之间**（守候 65766 实测，168/168），这一窗约 17 分钟。
  数据根克隆在 `data-root-at-recovery-20260913-115906/`。
- 恢复前几秒新起的进程里**没有 CacheDelete 扩展**，所以第 6 次那条线索这次没复现。
  这次出现的是 12:16:27 的 `authorizationhost.bundle`（Security.framework 的授权宿主）
  和一串 `mdworker`。第 6 次恢复前 7 秒也起过一个 Security 系的 `trustevaluationagent`。
  **两次恢复前都起了 Security.framework 家族的进程**：n=2，名字不一样，只是相关，
  记下来当下一窗要对照的项。
- 12:16:28 还有一个 session 在跑 `log show … "deny"`，那是人为的查询，不是系统动作，别误读。

## 第 8 次发作（12:54 前后起）

- 起点不晚于 12:54:49：「账读不出时退避叫醒」那个 worker 挂的守候（pid 88245）起守时
  读得动 17 个、读不动 151 个。离第 7 次恢复约 38 分钟。
- 13:01 复测：仍在发作。`directory` 回「通讯录读不出来」，`list_sessions` 回「暂无快照」，
  所以**故障期间机长停不了 session，也查不了号码**。
- 界面进程 18872（0.1.37 新二进制）读 approvals.json 同样 EPERM，**这一窗产品侧也瞎**，
  不只是 agent 侧。
