...
3. 独占锁一直持有到命令进程组退出、复验完成。PendingCrew 新 session 和自动额度/模型探针启动期间无法越过这把锁。命令无 shell 拼接，stdout/stderr 写入完整日志，显示末尾 32 KiB；登录 shell PATH 解析也改走该执行器（5 秒上限，失败回落既有路径兜底），避免版本探针之前无限等待。15 秒版本探针、10 秒进程探针、30 秒 doctor、300 秒升级超时均报错，终止整个命令进程组。
