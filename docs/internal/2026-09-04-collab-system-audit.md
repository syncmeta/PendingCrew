...
- **常驻后台：代码默认已经翻到 daemon，但用户那条路还没验过。** 准确的口径是 —— 默认走 daemon 的代码与命令行下的闭环已经完成并有证据；**真实的双击启动、关闭、更新、重开，一条都还没验**。所以现在**不能说**「关掉 app、更新 app，session 不再断」，只能说「代码上应该如此，等验」。
...
1. **点一下（先别急，等叫你）**：一次真实的 GUI 验收（双击起 app → 关掉 → 看 session 是否还活着 → 重开是否回到原 session）。这条只能人做，我们不驱动图形界面。**但要等候选构建做出来之后** —— P5a 那条线现在还被一个 spawn 观测缺口挡着，这会儿验等于验一个还在动的靶子。已记成人类 Todo #1，到点了我们叫你。
