...
<!-- doc-ref-base: 07ca3d5 -->
- 基准提交：`ba0cb07`
- 采样时刻：2026-09-11 17:17 / 17:23，**症状正在发生时**采的
- 采样手段：`sample <pid> 3` / `sample <pid> 5`（纯命令行，没碰界面）
- 原始样本：`samples/2026-09-11-typing-lag/`（已留档；原来在 `/tmp`，那会被回收）
- 第二半（那三处浪费的归因、修法、边界）：`2026-09-11-typing-lag-chat-window-fix.md`
  —— 那份重新读了主线程调用树，把三处都**按样本归因**了（本文这张模块表没做祖先-后代
  判定，那份做了），并且修正了一句：`timelineEntries` 一次 body 求值被读的次数，
  样本里数出来是 6 处不同的读取点、合计占主线程 14.2%。
...
谁占着（**这份是重量过的**，第一次我用 `grep -E '[c]laude|[c]odex'` 数出「75 个进程
6.1 GB」，那把尺子把 Claude 桌面 app 的渲染进程也算成了 agent —— 数错了）：

| 谁 | 进程数 | 常驻内存 |
|---|---|---|
| claude CLI session | 17 | 4.55 GB |
| 网页渲染（WebKit.WebContent） | 107 | 4.43 GB |
| ChatGPT.app | 30 | 1.43 GB |
| PendingCrew 家族（界面 + daemon + 25 个 MCP helper） | 24 | 1.00 GB |
| Claude 桌面 app | 9 | 0.89 GB |
| WorkBuddy | 9 | 0.84 GB |
| Obsidian | 8 | 0.68 GB |
| 微信 + 企业微信 | 20 | 0.96 GB |
| codex | 5 | 0.16 GB |

**大约一半是我们，一半是别的软件。** 单项最大的一块是 107 个网页渲染进程的 4.43 GB，
跟 PendingCrew 无关。

> 常驻内存把共享库重复计了，这些是量级不是精确值；而且它**不含被压缩掉的部分**，
> 所以每一行都是下界。

**没有闲置 session 可清**：22 个 agent 进程，最后一次写出东西全在 54 分钟以内
（判据是 runner 自己的会话文件 mtime）。我一开始在群里说「去清闲置 session 腾内存」，
那是**在量之前就许下的承诺**，已经当众更正。

**helper 25 个 / agent 22 个** —— 差 3 个，可能是孤儿，但那点内存不是主因
（helper 自退已在 `ba0cb07` 修掉，要等下一次装版才全员换代）。
