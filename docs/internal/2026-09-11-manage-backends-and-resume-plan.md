...
> **2026-09-13 前提变更**：上面这段说的「住在后端里」恰恰是个接线错误——它写在
> `SessionHost.start`，默认模式下只有后台进程调，于是后台拿自己跟自己比版本、**每次**
> 都不换（不只第一次）。`06894b1` 把换代检查挪进界面进程（viewer 连后台之前跑），
> 鸡生蛋随之不存在：新界面启动时探到旧后台版本不同就换。**这是读代码推的，没实测。**
> 代码里那句 `chickenAndEggNote` 已删（除一条测试外无人引用，且内容已不成立）。
> 详见 `docs/internal/2026-09-13-startup-duties-wiring-fix.md`。
