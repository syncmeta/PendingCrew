...
## 时长：44 分钟只是一个样本，不是周期（2026-09-13 实测）

09-12 那次量到 44 分 19 秒，很容易被当成「这个故障大概持续三刻钟」。**不成立。**

09-13 这次：**00:24:04 第一次探到 EPERM，到 04:05:27 仍然 EPERM，已经 3 小时 41 分。**
中间三个哨兵各跑满 60 分钟（每 10 秒一拍），窗口一次都没开。

所以别拿上一次的时长做任何计划。两条实操结论：

- **窗口一开先搬账**（`scripts/grab-ledgers.sh`），因为下一个窗口可能是几小时之后。
- 报「它会自己好」时要连这个跨度一起说，否则听的人会以为等一会儿就行。

## 第二个窗口的时长：9 小时以上（2026-09-13）

- 起：00:24:04 起读全部 EPERM（上一段记的 44 分 19 秒是**前一天那个**窗口，不是周期）。
- 止：09:29:39 探到可读，09:29:47 五本账全部搬出。
- 中间七个 60 分钟哨兵、10 秒轮询，一次都没探到开。
- 同样**没做任何解除动作**——没改权限、没 tccutil、没重启 app。

所以「44 分钟」不是这个故障的特征时长。两个样本：44 分 19 秒、> 9 小时。
**别再拿时长去猜它是什么。** 唯一稳的操作是上一段那条：窗口一开先搬账。
