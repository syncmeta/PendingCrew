...
### 4.3 单元 3（设置「后端」页：实况 + 重启入口）

- 判定层 `9c6434e`：`BackendRegistry.liveStatus(of:)` 四态（管不了 / 没在跑 / 问不出 / 在跑），
  **远程与非本数据根的 socket 一律不探**；`restartAction(for:appBuild:interfaceRelaunchesBackend:)`
  定按钮叫什么（viewer 里是「重启后台」/「换成 X」，不叫「停用」——停完界面会马上再拉一个）
  与确认框原话。顺手修了 `probeState` 的数：`hello.sessionCount` 混着已退出的。
- 界面：每行显示版本 / pid / 在跑几个（已退出另列）/ 与界面版本不一致时标橙；「刷新实况」；
  按钮与确认框全问模型层。重启走 `SessionHost.restartLocalBackend()`：同一个探针、
  同一个 `BackendUpdatePlan.decide`、同一句换代公告、同一个 `DaemonStopper`；**先说再停**；
  版本不同且停成功才问接回（同版是人自己按的，规格不问，确认框已提前说）。
- 设置窗注入了 `sessionHost`（不注入一读就崩）。

**单元 3 的边界**：
- 真停真起没跑过；界面长什么样没人看过（界面自动化禁令），只有接线源码断言 + 构建。
- 本机探针握手在主线程上，「刷新实况」与打开设置页会顿最多 2 秒。
- 重启后 3 秒再探一次实况；新后台 3 秒内没握上手的话会显示「没有后台进程在跑」，
  再点一次「刷新实况」才对。
- 手动同版重启的群公告文案在 `SessionHost` 里、没有测试（换代那句在 `BackendUpdatePlan`、有测试）。
- 「停止后台」（非 viewer）那一档停完不会自动再起，inproc 模式下才会出现。
