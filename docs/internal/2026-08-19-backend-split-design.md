...
#### 5.3.1 实测补正（P3 落地时改的，别照上面那六步的字面去做）

**① 第 4 步做不到「逐格」—— SwiftTerm 只公开活跃那一份缓冲区。**（2026-08-26 复核，机长 + 父机长拍板接受这处降级。）

`Terminal.buffer` 是 `public private(set)`，永远指向**当前活跃**的那一份；按 kind 取 `Buffer` 的 `bufferFromKind(kind:)` 没有 public。公开出来的只有 `getBufferAsData(kind:)`，返回 `Data` —— **只有文本，没有属性**。

于是：**正处在 alt 屏时，主屏那份历史只还原得了文本，还原不了颜色。** 影响面就这一句 —— 文本一个字不丢；不在 alt 屏时全程逐格精确。

被否掉的两条替代方案，别再提：**fork SwiftTerm 加访问器**（长期负债，每次升级带补丁，为一段被 TUI 盖住的 shell 历史的颜色不值）、**在 core 那侧扫字节认出 `?1049h` 提前存一份**（序列被拆成两批读就会漏，那种雷炸的时候没人查得到是这儿）。

这条限制在 `TerminalSnapshotEncoderTests` 里是**显式断言**，不是文档里的一句话。哪天 SwiftTerm 开放了非活跃缓冲区，那条断言会因为「颜色居然没掉」而红 —— 那就是回来收掉这处降级的线索。

**② 第 2/3 步「行尾 `\r\n`」是错的，会把折行拆散。** `BufferLine.isWrapped` 记的是「这一行是上一行自动折过来的」。给折行续行也塞一个 `\r\n`，它就变成了两行独立的行，改窗口宽度时再也不会合并回去 —— §7.2 当场红。**换行只在下一行不是折行续行时才发。**

**③ 「擦过的格子」既不是空格也不是没画过。** `\e[K` 填的是 code 0 + `eraseAttr()`（背景色留着）。填空格会把它变成 code 32（`getTrimmedLength` 从此裁不掉）、用 CUF 跳过去会丢掉那片背景色、什么都不做则整行被带色擦过时一格都不还原。**只有 ECH（`\e[nX`）与 `\e[K` 同源，只有它能原样还原。** 三种错法的**文本完全一样**，只看文本的比较会一律判绿 —— 所以 §7.1 必须逐格比属性。
...
**两条实测补正（P3 落地时改的，少一条就活锁 —— 上面那三行的字面读法正是把第一版锁死的那条）：**

1. **那个上限只管实时积压，不管快照。** 一份 10000 行回滚的快照本身就有好几 MB，比 2 MB 的上限还大。把快照也算进上限，就成了「排进快照 → 立刻超限 → 丢掉重排快照」的无限循环，**一个字节都发不出去**，而且**不报任何错** —— 表现只是「窗口一直空着」，没人会想到是背压逻辑在自锁。实测：600 批字节触发 **526 次重同步**。快照是必须发的载荷，丢掉它不解决任何问题。
2. **标脏之后不立刻拍快照，等 viewer 真来取的那一刻才拍。** 否则每来一批字节就重拍一次几 MB 的快照，而其中绝大多数在被人看到之前就已经被下一次重同步丢掉了。延后到取的那一刻拍，既省掉那些空拍，viewer 拿到的还是**当时最新**的那一份 —— 更省也更对。同样的场景 526 次 → 1 次。

由此还有一条**顺序依赖**，写反了会悄悄丢一批：每批 PTY 字节必须**先喂权威 `Terminal`、再排进待发队列**。反过来的话，标脏那一拍重拍的快照还没吃到这批字节，而这批字节又跟着队列一起被丢了。
...
2. **真 · agent TUI 录制**（fixtures）：把真实 session 的原始 PTY 字节流录下来存进 `Tests/Fixtures/`。随机语料测不出真实 TUI 的怪癖，而我们要还原的**恰恰就是它**。

   **录制器是 `AgentTuiFixtureRecorder`**（P3 落地，走 `AgentSessionCore`，全程无画面、不碰 GUI）。它默认 skip，只有带 `TEST_RUNNER_PENDINGCREW_RECORD_TUI=…` 才动 —— 它要花订阅额度、要联网、结果每次都不同，那三条里任何一条都足以让它不配当 CI 里的一条测试。**它是工具，产物才是测试。**

   **录哪两段（原文写的「claude 与 codex 各一段」是错的，别照着找）：**

   | 段 | 为什么 |
   |---|---|
   | `tui-claude.bin` | claude 走 PTY + 真终端，是这个编码器的主要服务对象 |
   | `tui-shell.bin` | `.terminal` 那种人直接用的 shell 也经过同一个编码器，但语料形状完全不同（没有整屏重绘，有真实的滚动历史与 `ls` 的颜色） |
   | ~~codex~~ | **不存在，也不该存在。** `CrewSessionRunner` 里 codex 走的是 `CodexAppServerBackend`（JSON-RPC），**没有 PTY、没有 `Terminal`**，输出是当结构化消息渲染的。codex 的字节一个也不会经过快照编码器 |

   **fixture 入库，缺失时 fail 不 skip。** 仓库里已经有一批「本机绿、别人静默 skip」的测试（`CrewChatOpenCostTests`，fixture 是 gitignored 的），那种绿是假的。这两段一共不到 20KB。

   **实测发现（会影响怎么读 §5.3.1）：claude 不进 alt 屏。** 它用主屏 + 大量列跳（`\e[nG`）+ 整行擦除（`\e[2K`）重绘。所以 §5.3.1 那条「alt 屏时主屏历史掉色」的降级，**对 claude 这条腿一次都不会发生**。有一条测试钉着这个事实，claude 哪天改了它会红。
...
| 阶段 | 做什么 | 完成判据 |
|---|---|---|
| **P5a**<br>把默认翻过去 | 真 daemon 冒烟（起 daemon → 真跑一个 session → 前端断开后仍活 → 重连 attach 恢复同一 session）；§8.4 订阅登录态实测；修掉冒烟暴露出来的阻塞项；最后才翻默认 | ✅ 2026-09-04 完成。闭环原始输出见 `2026-09-04-p5a-closed-loop-evidence.md`；**证据先于翻默认**（翻默认之后 claude / codex 两条闭环各复跑一遍，见该文档「翻默认之后的复跑」）。默认与 §9.2 契约同一笔落地，见 §9.2.1 |
...
### 9.2 翻默认之后「daemon 起不来」的硬契约（2026-09-04 由父机长拍板，**实现排在 P5a 两条绿之后**）

翻默认之前必须先把这条写死，因为它决定了**默认切过去之后最坏那一天长什么样**。

**背景**：默认翻成 daemon 之后，GUI 进程的身份是 viewer。viewer 连不上时有两种走法，
两种都能翻车，而且翻法相反：

- 只连不退：用户双击图标，界面在、什么都不动、不报错 —— §3 里已经点名过，
  这是这条线上**最贵**的翻车形状。
- 连不上就自己接管：如果那边其实**有**一个 daemon 在编排（只是这次没连上），
  就当场造出双头 —— 两个进程写同一批账、同一批唤醒发两遍，事后极难定位（§6.1）。

所以不是二选一，是**按「能不能确定没有别人在编排」分岔**。判据用现成的
`SessionOrchestratorLock.Outcome`，不另造一套：

| 观测到的 | 允许回退本地编排？ | 界面 |
|---|---|---|
| `acquired` —— 锁到手，且拉 daemon 确实失败 | ✅ **允许**（这是唯一允许的一支） | **持续**显示「后台起不来，已临时由本窗口接管」+ 原因 + 重试 |
| `heldBy(holder)` 且 `holder.kind == "daemon"` | ❌ 禁止 | 连上去（`followDaemon`，现有行为） |
| `heldBy(nil)` —— 锁被占着但读不出是谁 | ❌ **禁止**（归属不明 = 不许接管） | 可操作错误 + 重试 / 诊断 |
| `heldBy(holder)` 且 kind 不是 daemon | ❌ 禁止 | 冲突（`conflict`，现有行为） |
| `unavailable(reason)` —— 锁文件打不开 | ❌ 禁止（拿不到锁就没资格当唯一所有者，见锁自己的注释） | 可操作错误 + 原因 |
| 连上了但**协议不兼容** | ❌ 禁止 | 可操作错误（§4.4 的降级仍按能力集取交集，但不许因此自己接管） |
| 连上了但 **attach 失败** | ❌ 禁止 | 可操作错误 + 重试 |

三条附加约束：

1. **回退期间界面持续显示当前模式与原因**，不是弹一下就没 —— 用户必须随时看得出
   「我现在跑在临时模式上」。现成的载体是 `OrchestrationNotice` +
   `OrchestrationNoticeBar`（P4 已有，且「拒绝必须有出口」那条测试钉着它）。
2. **恢复连接之后不许残留第二个 host**。回退期间起的那套编排在连上 daemon 时必须
   整个停掉并交还所有权 —— 半停一半留就是双头的另一种形状。
3. **先红后绿**：上表每一行都要有一条会红的测试；尤其「归属不明时禁止静默接管」
   这一行，要能证明把它改成允许时测试当场红。

**这条契约不许被「让用户少看见一个错误」的理由削弱** —— 静默本地接管带来的双头，
症状是账被两个进程交替覆盖、唤醒发两遍，比一个说得清的错误横幅贵得多。

#### 9.2.1 落地实况（2026-09-04，与翻默认同一笔）

**默认已翻**：`ProcessRole.resolve` 里不设总闸 = `.viewer`；`inproc` 变成显式的回退开关；
拼错的值按新默认走（安全的那一侧：会有一个后台起来管账，而不是两个进程一起管）。

上表落在 `OrchestrationFallback.decide`（纯判定），顺序那一段落在
`OrchestrationFallbackCoordinator`（**刻意放在 `Sources/Mac/LocalRunner`** ——
`ViewerSessionClient` 在 `Sources/Mac/Services`，不进 test bundle，而
「什么时候才许取锁 / 接管后有没有真的停腿 / 没接管有没有真的放锁」这三件事做错
都是安静地坏，必须有测试盯着）。界面态是 `OrchestrationNotice.localFallback` /
`.refused`，且**裁决压过「连没连上」**（归属不明不会自己变清楚，不许显示成
「正在连接…」那种看起来自己会好的样子）。

**附加约束 2 的落法，与原文措辞有一处收窄，明写在这里**：接管之后**不交还**，
回后台模式的唯一走法是**重开 app**。理由不是省事 ——

- 回退期间本进程已经在养真的 agent 子进程，**它们是这个进程的孩子，交不给 daemon**；
  半路把编排交还就是这一节点名的「半停一半留 = 双头的另一种形状」。
- 而「不出现第二个 host」这条本身是**结构上**满足的，不靠谁记得去检查：接管时
  我们**持有编排锁**（任何 daemon 都会因此拒绝启动）并把 viewer 那条腿整个停掉，
  所以「本地编排 + 连上的 daemon」这个状态压根到不了。

代价是「重试」按钮退化成「重开 app」，横幅上就是这么写的。真要做**在线**交还，
前提是先有一条能把在跑的 agent 子进程交接出去的路 —— 那是另一期的事。
