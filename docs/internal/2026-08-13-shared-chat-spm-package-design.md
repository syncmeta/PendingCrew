...
> ### 这份文档现在的状态
>
> 原文状态行（`---` 下方第 4 行）写的是「**设计已获机长认可（2026-08-12 机长），尚未动工——挪文件需机长在机器还原完成后另行放行**」。**恢复不改变这一行的任何一个字，也不构成放行。** 截至 2026-08-20，`apps/shared/PendingChatKit/` 不存在、一个文件都没挪过。
>
...
**日期**：2026-08-13
**状态**：设计已获机长认可（2026-08-12 机长），**尚未动工**——挪文件需机长在机器还原完成后另行放行
