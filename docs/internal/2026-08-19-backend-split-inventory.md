...
本附录只依据当前仓库 Swift 源码做静态盘点；没有编译、运行 app 或启动 GUI。行号以 `24a7893`（`docs(split): 前后端分离调研清单`，即写下这份清单的那个提交）为准，头部的 `doc-ref-base` 就是它；**照今天的树去找这些行号会找不到，那不是引用烂了，是它描述的是那一刻**——要读请 `git show 24a7893:<path>`。这份文档后来被移动过、也被补过一行，但 base 说的是**被引用的那棵树**，不是这份文档自己的历史，所以它不变。这里的“持有”指对象、`Task`、`Timer`、订阅或连接的生命周期由谁控制，不代表逻辑一定定义在该文件。
