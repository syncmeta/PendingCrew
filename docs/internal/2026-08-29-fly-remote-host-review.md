...
>   —— **这一条指的是 P4 分支 `pendingcrew/session-49b1c7`，不是本文头部的基线
>   `d7ce988`：那棵树上还没有这个文件**。所以它刻意留成裸文件名，不补路径 ——
>   检测器看不见它是对的，它本来就不在被检测的那棵树上），
...
> **2026-08-30 修订 —— 人类 #91 定了范围，本文档的「Fly 专项」外壳作废**（架构结论不变）：
>
> - **Fly 不特殊，只当一台普通 Linux 主机。** 产品要的是「**添加任意 Linux 远程主机**」，
>   Fly 只是其中一台。因此 **§2 B-4（Fly 供给）与 §4 的 F3（镜像 / Machines API / 拉起销毁）
>   整条出范围** —— 机器怎么来是用户自己的事，我们不管 fly 账号、不管机器生命周期。
> - **§2 B-1 / §6 第 1 条（远端 agent 登录态）不再是现在要拍的技术方案。**
>   产品口径已定：**添加一台主机时，在那台主机上做一次登录 / 配对。** 这与本文原推荐的选项 C
>   同形，只是不再需要现在在 A/B/C 之间选型。真做到实现阶段再按实际阻塞提一条简明 Todo。
> - **§6 第 2 条（Fly 成本口径）、第 3 条（手机连本机 / NAT 拆项）随范围收窄而作废。**
>   §4 的 F6 同理：它是「手机连不在公网上的主机」这一类问题，不在当前范围内。
> - **不变的部分**（这也是这份复核真正的产出）：§3 的架构与四条纪律、§1.2 两层删除的红线、
>   §1.3 那份 iOS 通道设计已作废、以及 F1 / F2 / F5 三个阶段。把文中所有「Fly machine」
>   读成「远程 Linux 主机」即可，一个字都不用改。
...
| ~~**F3** Fly 供给~~ | ~~镜像、Machines API 拉起/销毁~~ | **2026-08-30 按 #91 出范围**：机器怎么来是用户的事，我们不管 fly 账号与机器生命周期 | —— |
| **F4** 添加一台主机 | 「添加远程 Linux 主机」的流程：地址 + 在**那台主机上**做一次登录/配对 | **起一个远端 session，让它跑一次真需要登录态的动作并成功**（「进程起来了」不算过，照抄 §8.4 的判据） | 本 crew |
| **F5** 手机连远程主机 | iOS viewer：`RemoteSessionBackend` + 终端镜像 + 群聊/审批面 | 手机上看到并驾驶一个跑在远程主机上的 session；**每一栏要么有真数据要么显式报错，不许空态**（对着「三次空管子」那条教训） | 本 crew |
| ~~**F6** 手机连本机~~ | ~~NAT 穿透 / 中转~~ | **2026-08-30 按 #91 出范围**：属「连不在公网上的主机」那类问题，当前不在范围内 | —— |

排期纪律：**F1 → F2 → F4 串行**（每一阶段都是下一阶段的地基）；
F5 必须在 F1 / F2 / F4 全绿之后才开工——否则又是一根空管子。
...
## 6. 待人类拍板 —— **已由 #91 结清，本节不再有待办**

原本列的三条（远端 agent 登录态选型 / Fly 成本口径 / 手机连本机是否留在 #44）**全部作废**：

- 范围已定：**Fly 不特殊，只当一台普通 Linux 主机**；机器怎么来不归我们管，所以成本口径失去范围。
- 登录态已给产品口径：**添加一台主机时在那台主机上做一次登录 / 配对**，技术方案不在现在拍。
- 「手机连本机」不在当前范围内。

**新纪律：不再为这条线预留待拍板项。** 真做到 F1 / F2 / F4 遇到实际阻塞时，再按那个具体阻塞
提一条简明 Todo —— 而不是提前把选择题摆到人面前。
