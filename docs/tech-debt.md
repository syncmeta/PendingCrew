...
2026-09-25 在 macOS 27 实测，贴底标定时内容确实增长，但 `.top` 和 `.bottom`
的视口位移都为 0；旧的 707pt 标定结果不能外推到此系统。两条依赖标定的探针在
当前系统明确 skip，不把「分不出」写成「锚无效」的通过。产品实际使用的
`.scrollPosition` 路径仍由同文件的首帧和终点用例直接验证。
...
**仍然不自动回收**，只在末尾多报一行说明大头是它。~~

**当天下午改了这个决定，因为量到了数**：DerivedData **每趟 2.1 GB**，而真正要留的
xcresult **只有 133 MB —— 16 倍**。当初留着它是为了「同一个 commit 复跑能热启动」，
那个好处一年用不上几次；而这道闸门每次发版、每次合前基线都跑一趟，**当天两趟就
4.3 GB**。现在三趟 build 跑完、xcresult 归档之后就 `rm -rf "$DD"`，
三份 .log ＋ drift.log 已经接住了 xcodebuild 的全部输出，扔掉它不会让任何读数查不了。

**这条的教训是「先量再定」**：第一版的取舍是在没有数字的情况下做的，
而拿到数字之后它当场就不成立了。
...
## ~~`quietLayouts` 恒读 0，两条防线因此是空绿~~（2026-09-12 发现 → 同日证伪 → **同日再被推翻一次**）
...
### 后来又逐条看了注入/工具那一批（2026-09-12 09:10）

判据换了个问法：**这一处会不会把「我读不到」变成一个看起来正常的业务事实？**
按这个问法查了 Mcp 那一批，三处真会，都已修：

| 处 | 读不出来时它会 | 修法 |
| --- | --- | --- |
| `search_whiteboard` | 说「当前 crew 没有找到匹配消息」 | 认出那一行警示，如实说读不出来 |
| `WhiteboardCursor.unread` / `advance` | 把警示当新消息投出去，并把游标钉到一个磁盘上没有的 id 上；**那条 id 的时间戳是「此刻」，恢复后比它旧的消息全被判成已投过** | 这一拍不投、不推、不 resync；`advance` 拒绝推到那个 id |
| `McpTurnHook` | 代发一条据此算出来的留痕，并把**本轮边界**落盘成那个不存在的 id | 这一拍不代发、不动边界 |

判据收在 `LocalWhiteboardStore.readFailure(in:)` 一份，三处共用。

**看过但**故意**没改的一处**：`HookEmitter.staleHint` 用 `CockpitPlanStore.list`，
读不出来时 `plans` 为空 ⇒ 那条陈旧度提示这一拍不发。它**不产生假陈述、也不落盘任何
错状态**，只是少发一次提示，而故障期间本来就什么都送不出去。改它要动注入面的行为，
收益对不上。

**其余那些（侧栏末条、未读数、展示路）仍然没有逐条看过。**
「从名字看是渲染路」不是「查过」——别把这一条读成「剩下的都审过没事」。
...
### 那条读数不稳，而我上午拿它当了事实（2026-09-12 09:15 补）

上午的结论是「`SwiftUIRepeatForeverDots` 在聊天骨架里已经不自激（读 0）」，据此做了两件事：
把那条写成 `XCTAssertLessThan(n, 10)`，并把正向对照的标本从 dots 换成
`TodoCircleAsCrashed`。**CI 当天就红了（19555），几小时后同一台开发机自己也翻到六万多**
（单跑三趟：66717 / 66308 / 66888）。

| 何时何地 | 读数 |
| --- | --- |
| 开发机 上午 | 0 |
| CI runner 同日同 commit | 19555 |
| 开发机 09:15 单跑三趟 | 66717 / 66308 / 66888 |

**所以问题不是「两台机器不同意」**（我一度在群里这么说，错了）—— **是这个读数自己都复现不了**。
一次读数在这里既不构成跨机器判据，也不构成同机判据。

**处置**：那条改成只打印不断言（`testRecordSwiftUIRepeatForeverDotsInChatShape`）。
正向对照**不改回 dots** —— 换上去那个标本今天几趟都稳定在几万量级，比 dots 可靠。

**可复用的那句**：**「我量到 0」和「它是 0」之间隔着「我量了几趟」。**
这一条上我连量第二趟都没做就写进了断言和判定。
