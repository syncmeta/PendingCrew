...
|---|---|---|---|---|
| 2026-09-07 | 共享树 `e4033dc`，机器较空 | passed | 5.445s | **未量**（当时没数——第一行就缺这一栏，正好说明这张表为什么需要它） |
...
| 日期 | 环境 | 结果 | 耗时 | 同时在跑的 xcodebuild |
|---|---|---|---|---|
| 2026-09-07 | 共享树 `e4033dc`，机器较空 | passed（`test_打开LED驱动板一次重排在预算内`；同批 8 条全部 started、全过） | 0.324s | **未量**（当时没数——第一行就缺这一栏，正好说明这张表为什么需要它） |
