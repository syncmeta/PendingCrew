...
- **2026-09-05 第三次：按上面那条当场抓了，边界量出来了 —— 不是「某个文件」，是
  「整个数据根的文件内容」。** 这次终于有了正面证据（前两次全是排除项）：
  ...
  **形状是「元数据放行、内容拦截，读写都拦，且只拦这一棵子树」** —— 这正是 macOS TCC
  保护 `~/Library/Application Support/<App>/` 的特征，不是 POSIX 权限、不是文件损坏
  （两次排除项这下有了正面解释）。

- **而且它是「跑着跑着断的」，不是一开始就没有。** 同一个 session 内：
  `01:20Z` 读 `local-crews.json` 成功 → `01:43Z` 又成功（逐条数 crew 工作目录）→
  `~01:50Z` 起全部 `Operation not permitted`。**中间我没换进程、没改权限。**
  所以不是「这个 session 从来没拿到过授权」，是**授权在会话中途没了**。

- **责任进程链（第三样证据）**: `zsh(14709) ← claude(5563) ← PendingCrew(95591)`。
  TCC 的授权挂在**责任进程**上，所以下次要查的是：那一刻 PendingCrew 这一侧发生了什么
  （新实例接管？授权被重置？有没有弹过 TCC 框而没人点？）。**这是从「查文件」转向
  「查责任进程」的关键一步** —— 前两次都在文件那头找，找不到是应该的。
