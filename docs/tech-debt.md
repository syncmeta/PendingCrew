...
### 🟢 `docs/architecture.md` 被 #63 两期打成过时，两次都**没改**
- **发现**: 2026-08-25 · Todo #63 第一期；2026-08-26 第二期又扩大一次
...
- **第二期又打过时了哪几行**（2026-08-26，只登记不动手，同样的理由）:
  - **34 行**「云端那半（Supabase 登录、跨设备遥控）代码在仓库里」—— 代码已经不在了。
  - **104 行**引用 `Sources/Support/KeychainStore.swift`（已删）。
  - **302 / 346 行**的目录表还列着 `Sources/Remote/`（3 个文件 / 634 行，已整目录删除）。
  - **764 行**的「11 个 skip」表里那条 `FamilyCredentialStoreTests.testSetGetClearRoundtrip`
    —— 测试和被测类型都删了，**干净 clone 的 skip 数从 11 变成 10**。这行现在会误导
    任何拿它对账测试结果的人，是本条里最该先改的一行。
  - **816 行**把 `CrewRelayAgent` 列进长期服务；**826 行**引用已结掉的那条 🔴。
- **该怎么还**: 重跑一遍那节的实测（`Package.resolved` 的 pin 数、编译出的模块数、`.o` 体积），把「云端登录那两族」整节删掉或改写成「已随 #63 移除」，清掉 `CrewHostedConfig` / `Sources/Remote/` / `KeychainStore` 的引用，并按实测重数 skip。**别照抄旧数字改个百分比** —— 那节的价值就在于它是实测出来的。
...
### 🟡 `CrewChatOpenCostTests.test_窗口内12条eager布局仍在预算内` 这条**性能预算断言在飘**
- **发现**: 2026-08-26 · 侧栏「手动藏起来一个 crew」落地时取全量基线，撞上前提对不上
- **位置**: `Tests/PendingCrewTests/CrewChatOpenCostTests.swift:493`（`XCTAssertLessThan(…, 100.0)`）
- **两次实测，写具体那一趟的结果，不写「与基线一致」**:
  - **2026-08-25**（Todo #63 第一期报告，`main` @ `e17b268` 与 `ab0942c` 两趟）：**红**，`112.464084` / `111.977917`，都 `not less than 100.0`。
