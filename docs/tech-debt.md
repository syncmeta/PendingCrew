...
## `wakeBusyStallAlert` 本机全历史 0 次触发 —— 未区分「没发生」与「触发不了」
...
## B3 只治「看得见的丢」——`pinPosition` 返回 `.retryLater` 那条路磁盘上零痕迹
...
## 发版闸门的 xcresult 没被归档 —— **留得住日志的是日常跑，留不住的恰恰是最要紧那一趟**
...
## `quietLayouts` 恒读 0，两条防线因此是空绿（2026-09-12 发现，未修）

**现象**（`main@7fe513d`，干净 detached 树全量的原始日志）：

```
[quietLayouts]     安静窗口内 layout() 次数 = 0     ← 三次调用，三次都是 0
[quietTodoLayouts] 安静窗口内 layout() 次数 = 0
[quietTodoLayouts] 安静窗口内 layout() 次数 = 39137  ← 同一趟里它测出了真数
```

`LayoutLoopRegressionTests` 里 `quietLayouts` 的**三次调用全部回 0**，
而**同一趟**里姊妹函数 `quietTodoLayouts` 测出了 39137。
**所以这不是「无头环境测不出来」** —— 那个技术在这台机器上是work 的。

### 代价：三条测试里两条在空绿

| 行 | 断言 | 读到 0 时 |
|---|---|---|
| `:134` 这个组合**本该**自激（`GreaterThan 1000`） | 要求大数 | **红** —— 这是这套测量里唯一还会说话的 |
| `:144` 呼吸红点**不许**自激（`LessThan`） | 要求小数 | **空绿**（0 当然小于阈值） |
| `:154` 打字点**不许**自激（`LessThan`） | 要求小数 | **空绿** |

**两条真防线现在什么都不保证**，而它们看起来是绿的 ——
这正是 [[a-check-that-never-fires]] 那个形状：「没触发」和「触发不了」长得一样。

### 为什么没有当场改

`:134` 的失败信息自己写着：「要么 SwiftUI 修了这个坑，要么这条测试的判据失效了，
**两种都要人来看一眼**」。**这条要的是人的判断，不是我顺手改个阈值。**

可以先做、且不需要判断 SwiftUI 的那一半：**给那两条 `LessThan` 加一道
「测量函数得先证明自己活着」的前置**（比如同一窗口里放一个已知会自激的对照）。
但那会让它们在当前环境**立刻变红**，是否现在就让主干红掉，是团队的决定，
不该由一个此刻联系不上 crew 的机长单方面做。

**不是今晚引入的**：`LayoutLoopRegressionTests.swift` 上次被人碰是 `a61e7a5`（08-15）。
