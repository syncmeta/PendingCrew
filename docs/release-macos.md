...
| 版本 | 闸门 | 结果 |
| --- | --- | --- |
| 0.1.22 | 跑了，钉在 `16b8182` | **1739 例 / 3 skip / 0 失败**，`** TEST SUCCEEDED **`，macOS 与 iOS Simulator build 都过。3 条 skip 逐项是 `AgentTuiFixtureRecorder.testRecord`、`CrewLastMessageCacheTests.test_基准_现场白板目录`、`SessionAwaitingReplyInputsCacheTests.test_基准_现场目录`；`CrewChatOpenCostTests` 8 条真执行；具名失败为空；HEAD / TREE 前后逐字相同。**真全绿。** |
