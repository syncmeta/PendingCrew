...
**Do not reach for the expensive models casually** (the user's own rule, 2026-09-12): the top tier on each side — claude's **fable**, codex's **astra** — costs noticeably more. **Default to the ordinary models.** Only three cases justify the expensive one: accuracy really matters here, the task is genuinely beyond an ordinary model, or it needs many rounds of thinking to converge. Important does not mean expensive — most important work runs fine on an ordinary model. When unsure, start ordinary and raise it with `set_session_profile` if you actually hit a wall, so the cost lands only on the part that needed it.
